...
contract BitwiseOps {
    ...
    // xor returns 1 when only one of the bits are one
    // x     = 1100 = 8 + 4 + 0 + 0 = 12
    // y     = 0101 = 0 + 4 + 0 + 1 = 5
    // x ^ y = 1001 = 8 + 0 + 0 + 1 = 9
    ...
    // use uint8 here below so the flipped number is not too large!
    ...
    function getLastNBits(uint256 x, uint256 n) external pure returns (uint256) {
        ...
        // mask     = 0111 = 7
        // x & mask = 0101 = 5
        // to make the mask: 1 --> 1000 -1 -> 0111
        uint256 mask = (1 << n) - 1;
        return x & mask;
    }
    ...
    function getLastNBitsUsingMod(uint256 x, uint256 n) external pure returns (uint256) {
        // 1 << n = 2 ** n ... 2 ** n is the same as 1 << n
        return x % (1 << n);
    }

    // Get position of most significant bit
    // x = 1100 = 10, most significant bit = 1000, so this function will return 3
    function mostSignificantBitViaShiftRight(uint256 x) external pure returns (uint256) {
        uint256 i = 0;
        while ((x >>= 1) > 0) {
            ++i;
        }
    ...
    /**
     * input of uint256 has 256 positions. The biggest position is at 255 as counting from right most bit from 0, 1, 2, ... which can be represented in uint8
     */
    function mostSigBitViaBinarySearch(uint256 x) external pure returns (uint256 msb) {
        // x >= 2 ** 128, as 128 is half of 256
    ...
    //This assembly code can save about 1500 gas
    function mostSigBitViaAssembly(uint256 x) external pure returns (uint256 msb) {
        assembly {
