...
import "src/ConstProductAMM.sol";
import "src/ERC20Token.sol";

contract ConstProductAMMTest is Test {
    ...
    uint256 tokAmt1;
    uint256 tokAmt1B4;
    address constProductAMMAddr;
    address token0Addr;
    address token1Addr;
    ConstProductAMM constProductAMM;
    ERC20Token token0;
    ERC20Token token1;
    address senderM;
    ...
    function setUp() public {
        ...
        constProductAMM = new ConstProductAMM(token0Addr, token1Addr);
        constProductAMMAddr = address(constProductAMM);
        tok0Approved = 1000;
        tok1Approved = 1000;
        token0.approve(constProductAMMAddr, tok0Approved);
        token1.approve(constProductAMMAddr, tok1Approved);
        vm.stopPrank();
    ...
    function test1() public {
        ...
        vm.prank(alice);
        constProductAMM.addLiquidity(tokAmt0, tokAmt1);
        shareBalc = constProductAMM.userShares(alice);
        addedShares = constProductAMM.sqrt(tokAmt0 * tokAmt1);
        console.log("Alice shareBalc: ", shareBalc);
        assertEq(shareBalc, addedShares);
        shareBalcB4 = addedShares;

        totalShares = constProductAMM.totalShares();
        console.log("totalShares: ", totalShares);
        assertEq(totalShares, addedShares);

        res0Balc = constProductAMM.reserve0();
        console.log("res0Balc: ", res0Balc);
        assertEq(res0Balc, tokAmt0);

        res1Balc = constProductAMM.reserve1();
        ...
        res0BalcB4 = constProductAMM.reserve0();
        res1BalcB4 = constProductAMM.reserve1();

        vm.prank(bob);
        swapAmtOut = constProductAMM.swap(token0Addr, swapAmtIn);
        ...
        vm.prank(alice);
        constProductAMM.removeLiquidity(shareWithdrawn);
        shareBalc = constProductAMM.userShares(alice);
        ...
        console.log("end of ConstProductAMM");
    }
}
